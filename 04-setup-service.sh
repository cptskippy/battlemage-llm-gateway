...
# 03-discover-models.sh) and builds llama-swap.yaml dynamically.  This 
# script will ignore models with a corresponding .ignore file.
#
...
while IFS= read -r -d '' gguf; do
  ignore_file="${gguf}.ignore"
  params_file="${gguf}.llama.cpp.params"

  if [[ -f "$ignore_file" ]]; then
    info "Model $gguf is flagged as ignored — skipping."
    continue
  fi
